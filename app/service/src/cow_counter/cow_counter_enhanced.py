...
def enhanced_cow_detection(image_path, output_dir="output"):
        ...
        try:
            ...
            for conf_thresh in confidence_thresholds:
                ...
                for grid_info in grid_sections:
                    grid_results = model(
                        grid_info['section'], conf=conf_thresh, verbose=False)
    ...
    create_detection_visualizations(
        original_image, final_detections, output_dir, fig, axes)

    # Save results
    save_detection_results(image_path, final_detections,
...
def create_detection_visualizations(image, detections, output_dir, fig, axes):
        ...
        if row < 2 and col < 3:
            ...
            if zoomed_section.size > 0:
                axes[row, col].imshow(cv2.cvtColor(
                    zoomed_section, cv2.COLOR_BGR2RGB))
                axes[row, col].set_title(
                    f'Cow {i+1} (Conf: {detection["confidence"]:.2f})')
    ...
    plt.savefig(os.path.join(
        output_dir, 'detection_analysis_enhanced.png'), dpi=300, bbox_inches='tight')
    plt.close()

    # Save annotated result image
    cv2.imwrite(os.path.join(
        output_dir, 'annotated_result_enhanced.jpg'), result_image)
...
def save_detection_results(image_path, detections, best_model, output_dir):
    ...
    api_json_file = os.path.join(
        output_dir, 'api_detection_results_enhanced.json')
    with open(api_json_file, 'w') as f:
        json.dump(api_format, f, indent=2)

    # Save text summary file
    summary_file = os.path.join(
        output_dir, 'enhanced_detection_summary_enhanced.txt')

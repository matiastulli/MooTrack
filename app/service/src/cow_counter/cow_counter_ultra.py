...
def visualize_results(image, detections, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'comprehensive_analysis_ultra.png'), dpi=300, bbox_inches='tight')
    plt.close()
    
    # Save the annotated image
    cv2.imwrite(os.path.join(output_dir, 'final_result_ultra.jpg'), result_image)
...
def save_detection_results(image_path: str, detections: List[Dict[str, Any]], best_model: str, output_dir: str = "output") -> str:
    ...
    api_json_file = os.path.join(output_dir, 'api_detection_results_ultra.json')
